...
kobo.service('$miscUtils', ['$rootScope', '$userDetails', function ($rootScope, $userDetails) {
    ...
    this.toggle_response_list = function (item, show) {
        if (item.type !== 'select_one' && item.type !== 'select_all' && item.type !== 'select_multiple' && item.type !== 'free_choice_field') {
            return;
        }
        ...
        if (item.meta.show_responses) {
            ...
            //TODO this should be un commented to show the options in multiple choice questions
