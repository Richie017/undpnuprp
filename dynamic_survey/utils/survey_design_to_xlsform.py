...
class KoboRankGroup(GroupHandler):
    ...
    def handle_row(self, row):
        ...
        if rtype == 'end rank':
            ...
            raise TypeError(
                "'%s': KoboRank groups can only contain rows with type='rank__level' (or 'end rank')" % row.get('type'))
...
class KoboScoreGroup(GroupHandler):
    ...
    def handle_row(self, row):
        if row.get('type') == 'end score':
            ...
            raise TypeError(
                "'%s': KoboScore groups can only contain rows with type='score__row' (or 'end score')" % row.get(
                    'type'))
