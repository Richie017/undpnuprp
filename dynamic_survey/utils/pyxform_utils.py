...
from dynamic_survey.utils.survey_design_to_xlsform import convert_any_kobo_features_to_xlsform_survey_structure


def _smart_unicode(maybe_utf):
    if isinstance(maybe_utf, str):
        return maybe_utf
    return str(maybe_utf, 'utf-8')


def create_survey_from_ss_struct(ss_struct, default_name='KoBoFormSurvey', default_language='default',
                                 warnings=None, ):
    dict_repr = xls2json.workbook_to_json(ss_struct, default_name, default_language, warnings)
    dict_repr['name'] = dict_repr['id_string']
    return builder.create_survey_element_from_dict(dict_repr)


def create_survey_from_csv_text(csv_text, default_name='KoBoFormSurvey', default_language='default', warnings=None, ):
    byte_csv_repr = BytesIO(str.encode(csv_text, encoding='utf-8'))
    workbook_dict = xls2json_backends.csv_to_dict(byte_csv_repr)
...
def convert_xls_to_csv_string(xls_file_object, strip_empty_rows=True):
    ...
    def _iswhitespace(string):
        return isinstance(string, six.string_types) and len(string.strip()) == 0
...
def convert_xls_to_csv_string_except_groups(xls_file_object, strip_empty_rows=True):
    """
    The goal: Convert an XLS file object to a CSV string.
    ...
    def _sheet_to_lists(sheet):
        result = []
        for row in range(0, sheet.nrows):
            current_row = [i.value for i in sheet.row(row)]
            if "begin group" in current_row or "end group" in current_row:
                continue
            row_results = []
            row_empty = True
            for col in range(0, sheet.ncols):
...
def convert_csv_to_xls(csv_repr):
    byte_csv_repr = BytesIO(str.encode(csv_repr, encoding='utf-8'))
    dict_repr = xls2json_backends.csv_to_dict(byte_csv_repr)
    return convert_dict_to_xls(dict_repr)


def convert_csv_to_ss_structure(csv_repr):
    # csv_repr_str = StringIO(csv_repr.encode("utf-8"))
    byte_csv_repr = BytesIO(str.encode(csv_repr, encoding='utf-8'))
    dict_repr = dict(xls2json_backends.csv_to_dict(byte_csv_repr))
    copy_dict = dict()
