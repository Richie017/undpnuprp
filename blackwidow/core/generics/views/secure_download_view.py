...
class GenericSecureDownloadView(GenericDetailsView):
    ...
    def generate_downloadable_content(cls, file_name, file_path, *args, **kwargs):
        ...
        if os.path.isfile(file_path):
            try:
                ...
                response = cls.response_class((line for line in open(file_path, 'rb')))
    ...
    def get(self, request, *args, **kwargs):
        ...
        upload_file_obj = UploadFileObject.objects.filter(pk=_document_pk).first()
        if upload_file_obj:
            file_obj = upload_file_obj.fileobject
            ...
            _content_name = upload_file_obj.original_name if upload_file_obj.original_name else file_obj.name
            response = self.generate_downloadable_content(
                file_name=_content_name,
                file_path=_file_physical_path
            )
            return response
        else:
