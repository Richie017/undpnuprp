...
from undp_nuprp.approvals.models import MonthlyProgress
...
@decorate(override_view(model=MonthlyProgress, view=ViewActionEnum.Manage))
class MonthlyProgressView(TargetBaseView):
    def get_template_names(self):
        return ['target_and_progress/monthly_progress.html']

    def get_search_parameters(self):
        parameters = super(MonthlyProgressView, self).get_search_parameters()
        today = date.today()
        year_choices = tuple()
        ...
        for _choice, _quarter in enumerate(MonthlyProgress.get_quarters()):
    ...
    def get_json_response(self, content, **kwargs):
        ...
        data_dict['header_widths'] = [5, 5, 5, 10, 5, 10, 10, 6, 6, 6, 6, 6, 5, 5, 5, 5]
        data_dict['data_rows'] = self.model.get_progress_data_rows(
            month=month, year=year, city_ids=city_ids, outputs=outputs, quarter=quarter)

        return super(MonthlyProgressView, self).get_json_response(self.convert_context_to_json(data_dict), **kwargs)
