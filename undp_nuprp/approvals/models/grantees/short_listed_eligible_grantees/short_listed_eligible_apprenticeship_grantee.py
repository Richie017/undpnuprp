...
@decorate(is_object_context, enable_import, route(
    route='shortlisted-eligible-apprenticeship-grantee', module=ModuleEnum.Analysis,
    group='Local Economy Livelihood and Financial Inclusion',
    group_order=3, item_order=6, display_name='Short Listed Apprenticeship Grantees'
))
class ShortListedEligibleApprenticeshipGrantee(ShortListedEligibleGrantee):
    class Meta:
        app_label = 'approvals'
        proxy = True
    ...
    def exporter_config(cls, organization=None, **kwargs):
        ...
        columns.extend([
            ...
            ExporterColumnConfig(column=column_no + 4,
                                 column_name='Is already receiving any skill training from other organization/s',
                                 property_name='other_grant_recipient', ignore=False),
            ExporterColumnConfig(column=column_no + 5, column_name='Student in formal education',
                                 property_name='other_grant_recipient', ignore=False),
            ExporterColumnConfig(column=column_no + 6, column_name='Eligible', property_name='render_eligible',
                                 ignore=False),
            ExporterColumnConfig(column=column_no + 7, column_name='Other Grants', property_name='render_other_grants',
                                 ignore=False),
            ExporterColumnConfig(column=column_no + 8, column_name='Name of Trade for which s/he haas interest',
