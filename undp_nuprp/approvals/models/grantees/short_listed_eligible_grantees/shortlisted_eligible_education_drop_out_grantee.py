...
@decorate(is_object_context, enable_import, route(
    route='shortlisted-eligible-education-drop-out-grantee', module=ModuleEnum.Analysis,
    group='Local Economy Livelihood and Financial Inclusion',
    group_order=3, item_order=7, display_name='Short Listed Education Grantees (Reduce Dropout)'
))
class ShortListedEligibleEducationDropOutGrantee(ShortListedEligibleGrantee):
    class Meta:
        app_label = 'approvals'
        proxy = True
