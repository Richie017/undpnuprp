...
class ViolenceAgainstWomanCommittee(OrganizationDomainEntity):
    ...
    def table_columns(cls):
        return [
            'code', 'city', 'number_of_violence_against_woman_committees',
    ...
    def details_config(self):
        d = OrderedDict()
        d['City'] = self.city
        d['No. of Violence Against Woman Committees'] = self.number_of_violence_against_woman_committees
    ...
    def importer_config(cls, organization=None, **kwargs):
        ...
        columns = [
            ...
            ImporterColumnConfig(column=1, column_name='Number of Violence Against Woman Committees',
                                 property_name='number_of_violence_against_woman_committees', ignore=False),
        ]
    ...
    def post_processing_import_completed(cls, items=[], user=None, organization=None, **kwargs):
        ...
        for index, item in enumerate(items):
            ...
            item_1 = cls.to_int(str(item['1']).strip(), 0)

            city_ = Geography.objects.filter(level__name='Pourashava/City Corporation', name__iexact=city).first()

            if city_:
                new_ = ViolenceAgainstWomanCommittee(
                    city=city_,
                    organization=organization,
                    number_of_violence_against_woman_committees=item_1,
        ...
        if create_list:
            ViolenceAgainstWomanCommittee.objects.bulk_create(create_list, batch_size=200)

        empties = ViolenceAgainstWomanCommittee.objects.filter(code='')
        update_list = []
        ...
        if update_list:
            ViolenceAgainstWomanCommittee.objects.bulk_update(update_list, batch_size=200)
