...
@decorate(is_object_context, enable_import, route(
    route='sef-apprenticeship-grant-disbursement', module=ModuleEnum.Analysis,
    group='Local Economy Livelihood and Financial Inclusion',
    group_order=3, item_order=15, display_name='Apprenticeship Grant Disbursement'
))
class SEFApprenticeshipGrantDisbursement(SEFGrantDisbursement):
    class Meta:
        app_label = 'approvals'
        proxy = True

    def get_or_create_sef_grant(self, pg_member, **kwargs):
        sef_grant = SEFApprenticeshipGrantee.objects.filter(
        ...
        if not sef_grant:
            ...
            if len(_ward) == 1:
                _ward = "0" + str(_ward)
            sef_grant = SEFApprenticeshipGrantee.objects.create(
                pg_member_name=self.pg_member_name,
                pg_member_assigned_code=self.pg_member_assigned_code,
                pg_member=pg_member,
                ward=_ward
        ...
        type_of_trade = kwargs.get('type_of_trade', '').strip()
        ...
        if type_of_trade:
            tt = TradeSector.objects.filter(name__iexact=type_of_trade).first()
            if not tt:
                tt = TradeSector(name=type_of_trade)
                tt.save()
            sef_grant.trade_sector = tt
        sef_grant.save()

    def soft_delete(self, *args, force_delete=False, user=None, skip_log=False, **kwargs):
        try:
            referred_grantee = self.sefapprenticeshipgrantee
            if referred_grantee:
