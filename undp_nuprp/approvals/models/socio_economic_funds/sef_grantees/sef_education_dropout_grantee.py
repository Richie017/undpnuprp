...
@decorate(is_object_context,
          route(route='sef-education-dropout-grantee', group='Local Economy Livelihood and Financial Inclusion',
                module=ModuleEnum.Analysis,
                display_name='Education Grantees (Addressing Dropout)', group_order=3, item_order=11))
class SEFEducationDropoutGrantee(SEFGrantee):
    education_level = models.CharField(max_length=128, blank=True, null=True)
    is_still_attending_school = models.CharField(max_length=128, blank=True, null=True)
    ...
    def table_columns(cls):
        return (
            "code", "name:Beneficiary Name", "render_contact_number", "render_PG_member_ID", "render_pg_member_name",
            "render_CDC", "render_city_corporation", "ward", "render_grant_disbursement_year",
    ...
    def details_config(self):
        ...
        basic_info['class'] = self.education_level
        basic_info['Is the grantee still attending School?'] = self.is_still_attending_school
    ...
    def export_file_columns(cls):
        ...
        """
        _table_columns = ["code", "name:Beneficiary Name", "render_contact_number", "render_PG_member_ID",
                          "render_pg_member_name",  "render_CDC_name", "render_CDC_ID", "render_city_corporation", "ward",
                          "relation_with_pg_member:Relationship of grantee to PG member", "render_grant_disbursement_year",
                          "date_created:Created On", "last_updated:Last Updated On"]
    ...
    def details_view_fields(cls):
        ...
        """
        return ['age', 'gender', 'education_level', 'is_still_attending_school', 'grantee_status', 'remarks',
                'difficulty_in_seeing',
                'difficulty_in_hearing', 'difficulty_in_walking', 'difficulty_in_remembering', 'has_disability',
                'difficulty_in_self_care', 'difficulty_in_communicating', 'render_total_installment']
