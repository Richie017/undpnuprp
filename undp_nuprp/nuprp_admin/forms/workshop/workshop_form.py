...
participant_info_choices = (
    ('', 'Select one'),
    ('LG', 'LG'),
    ('PG', 'PG'),
    ('Non-PG', 'Non-PG'),
    ('NUPRP staff', 'NUPRP staff'),
    ('Other', 'Other'),
)


class WorkshopForm(GenericFormMixin):
    def __init__(self, data=None, files=None, instance=None, prefix='', **kwargs):
        super(WorkshopForm, self).__init__(data=data, files=files, instance=instance, prefix=prefix, **kwargs)
        ...
        self.fields['workshop_date'] = forms.DateTimeField(
        ...
        self.fields['has_report'] = forms.CharField(
            label='Is a workshop report available, if required (Yes/ No)?',
            widget=forms.Select(
    ...
    def save(self, commit=True):
        with transaction.atomic():
            super(WorkshopForm, self).save(commit=True)
    ...
    class Meta(GenericFormMixin.Meta):
        model = Workshop
        fields = (
            'training_title', 'training_location', 'city', 'workshop_date', 'venue', 'facilitators',
            'number_of_pg_attendees',
    ...
    def field_groups(cls):
        _group = super(WorkshopForm, cls).field_groups()
        _group['A meeting at which a group of people engage in intensive discussion and activity on a ' \
               'particular subject or project'] = \
            ['training_title', 'training_location', 'city', 'workshop_date', 'venue', 'facilitators',
             'number_of_pg_attendees', 'number_of_non_pg_attendees', 'training_participants', 'participant_male',
             'participant_female', 'participant_total', 'duration', 'has_report']
