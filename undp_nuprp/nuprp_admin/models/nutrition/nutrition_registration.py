...
@decorate(is_object_context, route(route='nutrition-registration',
                                   group='Local Economy Livelihood and Financial Inclusion', module=ModuleEnum.Analysis,
                                   display_name='Nutrition Registration', group_order=3, item_order=21),
          enable_import, enable_export)
class NutritionRegistration(OrganizationDomainEntity):
    ...
    def post_processing_import_completed(cls, items=[], user=None, organization=None, **kwargs):
        ...
        for index, item in enumerate(items):
            ...
            if city_:
                old_ = NutritionRegistration.objects.filter(city=city_, month=month, year=year).first()
                if old_:
        ...
        if create_list:
            NutritionRegistration.objects.bulk_create(create_list, batch_size=200)

        if update_list:
            NutritionRegistration.objects.bulk_update(update_list, batch_size=200)

        empties = NutritionRegistration.objects.filter(code='')
        update_list = []
