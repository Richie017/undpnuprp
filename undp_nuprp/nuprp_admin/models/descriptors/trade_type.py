...
@decorate(is_object_context, enable_import,
          route(route='trade', group='Descriptors',
                module=ModuleEnum.Settings, display_name="Trade", item_order=4))
class TradeType(SubSector):
    class Meta:
        proxy = True
    ...
    def importer_config(cls, organization=None, **kwargs):
        ...
        columns = [
            ...
            ImporterColumnConfig(column=1, column_name='Trade', property_name='name', ignore=False),
        ]
        for c in columns:
    ...
    def run_post_processing_import(cls, items=[], user=None, organization=None, **kwargs):
        ...
        sector_max_seqs, created = MaxSequence.objects.get_or_create(context=TradeSector.__name__)
        sector_seqs_value = sector_max_seqs.value
        for sector in TradeSector.objects.all():
            ...
            try:
                ...
                if sector_name:
                    ...
                    if _sector_name in sector_dict.keys():
                        ...
                        time_now += 1
                        sector.type = TradeSector.__name__
                        sector.tsync_id = uuid.uuid4() \
                            if sector.tsync_id is None else sector.tsync_id
                        sector.last_updated = time_now
                        time_now += 1
                        sector.code = sector.code_prefix + sector.code_separator + str(sector_seqs_value).zfill(5)
                        sector_seqs_value += 1
        ...
        if len(sectors_create) > 0:
            TradeSector.objects.bulk_create(sectors_create)
        if len(sectors_update) > 0:
            TradeSector.objects.bulk_update(sectors_update)
        sector_dict = OrderedDict()
        for sector in TradeSector.objects.all():
            _sector_name = unidecode(sector.name).lower()
            sector_dict[_sector_name] = sector
        ...
        sub_sector_max_seqs, created = MaxSequence.objects.get_or_create(context=TradeType.__name__)
        sub_sector_seqs_value = sub_sector_max_seqs.value
        for sub_sector in TradeType.objects.all():
                ...
                if sector_name:
                    ...
                    if sub_sector_name:
                        ...
                        if _sub_sector_key in sub_sector_dict.keys():
                            ...
                            time_now += 1
                            sub_sector.parent_id = sector.pk if sector else None
                            sub_sector.type = TradeType.__name__
                            sub_sector.tsync_id = uuid.uuid4() \
                                if sub_sector.tsync_id is None else sub_sector.tsync_id
                            sub_sector.last_updated = time_now
                            time_now += 1
        ...
        if len(sub_sector_create) > 0:
            TradeType.objects.bulk_create(sub_sector_create)
        if len(sub_sector_update) > 0:
            TradeType.objects.bulk_update(sub_sector_update)
