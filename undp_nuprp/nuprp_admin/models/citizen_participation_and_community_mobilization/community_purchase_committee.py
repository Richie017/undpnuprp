...
@decorate(is_object_context, enable_import, enable_export,
          route(route='community-purchase-committee', group='Social Mobilization and Community Capacity Building',
                module=ModuleEnum.Analysis, display_name='Community Purchase Committee', group_order=2,
                item_order=5)
          )
class CommunityPurchaseCommittee(OrganizationDomainEntity):
    ...
    disabled_male = models.IntegerField(null=True, blank=True)
    disabled_female = models.IntegerField(null=True, blank=True)
    total_disabled = models.IntegerField(null=True, blank=True)
    total = models.IntegerField(null=True, blank=True)

    def save(self, *args, organization=None, **kwargs):
        self.total = (self.male or 0) + (self.female or 0)
        self.total_disabled = (self.disabled_male or 0) + (self.disabled_female or 0)
        super(CommunityPurchaseCommittee, self).save()
    ...
    def details_view_fields(cls):
        return [
            'detail_title', 'code', 'year', 'render_city', 'ward_no', 'trained:How many trained?', 'male', 'female',
            'total', 'disabled_male', 'disabled_female', 'total_disabled',
    ...
    def table_columns(cls):
        return [
            'code', 'year', 'render_city', 'ward_no', 'trained:How many trained?', 'male', 'female', 'total',
            'disabled_male', 'disabled_female', 'total_disabled', 'total_committee_formed:How many committee formed?',
            'created_by', 'date_created', 'last_updated:Last Updated On'
        ]
    ...
    def importer_config(cls, organization=None, **kwargs):
        ...
        columns = [
            ...
            ImporterColumnConfig(column=7, column_name='Total', property_name='total', ignore=False),
            ImporterColumnConfig(column=8, column_name='Disabled Male', property_name='disabled_male', ignore=False),
            ImporterColumnConfig(column=9, column_name='Disabled Female',
                                 property_name='disabled_female', ignore=False),
            ImporterColumnConfig(column=10, column_name='Total Disabled',
                                 property_name='total_disabled', ignore=False),
        ]
    ...
    def post_processing_import_completed(cls, items=[], user=None, organization=None, **kwargs):
        ...
        for index, item in enumerate(items):
            ...
            total = cls.to_int(str(item['7']).strip(), None)
            disabled_male = cls.to_int(str(item['8']).strip(), None)
            disabled_female = cls.to_int(str(item['9']).strip(), None)
            total_disabled = cls.to_int(str(item['10']).strip(), None)

            city_ = Geography.objects.filter(level__name='Pourashava/City Corporation', name__iexact=city)

            if city_.exists():
                new_ = CommunityPurchaseCommittee(
        ...
        if create_list:
            CommunityPurchaseCommittee.objects.bulk_create(create_list, batch_size=200)

        empties = CommunityPurchaseCommittee.objects.filter(code='')
        update_list = []
        ...
        if update_list:
            CommunityPurchaseCommittee.objects.bulk_update(update_list, batch_size=200)
    ...
    def exporter_config(cls, organization=None, **kwargs):
        ...
        columns = [
            ...
            ExporterColumnConfig(column=7, column_name='Total', property_name='total', ignore=False),
            ExporterColumnConfig(column=8, column_name='Disabled Male', property_name='disabled_male', ignore=False),
            ExporterColumnConfig(column=9, column_name='Disabled Female',
                                 property_name='disabled_female', ignore=False),
            ExporterColumnConfig(column=10, column_name='Total Disabled',
                                 property_name='total_disabled', ignore=False),
        ]
