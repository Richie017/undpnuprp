...
@decorate(is_object_context, enable_import, enable_export,
          route(route='community-scorecard', group='Social Mobilization and Community Capacity Building',
                module=ModuleEnum.Analysis, display_name='Community Score Card', group_order=2,
                item_order=4)
          )
class CommunityScorecard(OrganizationDomainEntity):
    ...
    matured = models.IntegerField(null=True, blank=True)
    functional = models.IntegerField(null=True, blank=True)
    ...
    def details_view_fields(cls):
        return [
            'detail_title', 'code', 'year', 'render_city', 'ward_no', 'established:How many established?',
            'matured:How many matured?', 'functional:How many functional?',
            'discontinued:How many discontinued?', 'created_by', 'date_created', 'last_updated:Last Updated On'
        ]

    @classmethod
    def table_columns(cls):
        return [
            'code', 'year', 'render_city', 'ward_no', 'established:How many established?',
            'matured:How many matured?', 'functional:How many functional?',
    ...
    def importer_config(cls, organization=None, **kwargs):
        ...
        columns = [
            ...
            ImporterColumnConfig(column=3, column_name='How many established?',
                                 property_name='established', ignore=False),
            ImporterColumnConfig(column=4, column_name='How many matured?',
                                 property_name='matured', ignore=False),
            ImporterColumnConfig(column=5, column_name='How many functional?', property_name='functional',
                                 ignore=False),
            ImporterColumnConfig(column=6, column_name='How many discontinued?', property_name='discontinued',
                                 ignore=False)
        ]
    ...
    def post_processing_import_completed(cls, items=[], user=None, organization=None, **kwargs):
        ...
        for index, item in enumerate(items):
            ...
            established = cls.to_int(str(item['3']).strip(), None)
            matured = cls.to_int(str(item['4']).strip(), None)
            functional = cls.to_int(str(item['5']).strip(), None)
            discontinued = cls.to_int(str(item['6']).strip(), None)

            city_ = Geography.objects.filter(level__name='Pourashava/City Corporation', name__iexact=city)

            if city_.exists():
                new_ = CommunityScorecard(
        ...
        if create_list:
            CommunityScorecard.objects.bulk_create(create_list, batch_size=200)

        empties = CommunityScorecard.objects.filter(code='')
        update_list = []
        ...
        if update_list:
            CommunityScorecard.objects.bulk_update(update_list, batch_size=200)
    ...
    def exporter_config(cls, organization=None, **kwargs):
        ...
        columns = [
            ...
            ExporterColumnConfig(column=3, column_name='How many established?',
                                 property_name='established', ignore=False),
            ExporterColumnConfig(column=4, column_name='How many matured?',
                                 property_name='matured', ignore=False),
            ExporterColumnConfig(column=5, column_name='How many functional?', property_name='functional',
                                 ignore=False),
            ExporterColumnConfig(column=6, column_name='How many discontinued?', property_name='discontinued',
                                 ignore=False)
        ]
