...
@decorate(is_object_context, route(route='vacant-land-mapping', group='Housing Finance',
...
class VacantLandMapping(OrganizationDomainEntity):
    ...
    vlm_approved = models.CharField(max_length=3, null=True, blank=True)
    ...
    def details_view_fields(cls):
        return 'detail_title', 'code', 'render_city', 'vacant_land_mapping_required', 'survey_completed', \
               'mapping_completed', 'vlm_approved:VLM Approved', 'created_by', 'date_created', \
               'last_updated:Last Updated On'

    @classmethod
    def table_columns(cls):
        return ['code', 'render_city', 'vacant_land_mapping_required', 'survey_completed',
                'mapping_completed', 'vlm_approved:VLM Approved', 'created_by', 'date_created',
                'last_updated:Last Updated On']
    ...
    def importer_config(cls, organization=None, **kwargs):
        ...
        columns = [
            ...
            ImporterColumnConfig(column=1, column_name='Vacant land mapping required',
                                 property_name='vacant_land_mapping_required', ignore=False),
            ImporterColumnConfig(column=2, column_name='Survey Completed',
                                 property_name='survey_completed', ignore=False),
            ImporterColumnConfig(column=3, column_name='Mapping Completed',
                                 property_name='mapping_completed', ignore=False),
            ImporterColumnConfig(column=4, column_name='VLM Approved', property_name='vlm_approved', ignore=False)
        ]
    ...
    def post_processing_import_completed(cls, items=[], user=None, organization=None, **kwargs):
        ...
        for index, item in enumerate(items):
            ...
            if city_.exists():
                new_ = VacantLandMapping(
                    organization=organization,
                    city=city_.first(),
                    vacant_land_mapping_required=item2,
                    survey_completed=item3,
                    mapping_completed=item4,
                    vlm_approved=item5,
        ...
        if create_list:
            VacantLandMapping.objects.bulk_create(create_list, batch_size=200)

        empties = VacantLandMapping.objects.filter(code='')
        update_list = []
        ...
        if update_list:
            VacantLandMapping.objects.bulk_update(update_list, batch_size=200)
    ...
    def exporter_config(cls, organization=None, **kwargs):
        ...
        columns = [
            ...
            ExporterColumnConfig(column=1, column_name='Vacant land mapping required',
                                 property_name='vacant_land_mapping_required', ignore=False),
            ExporterColumnConfig(column=2, column_name='Survey Completed',
                                 property_name='survey_completed', ignore=False),
            ExporterColumnConfig(column=3, column_name='Mapping Completed',
                                 property_name='mapping_completed', ignore=False),
            ExporterColumnConfig(column=4, column_name='VLM Approved', property_name='vlm_approved', ignore=False)
        ]
