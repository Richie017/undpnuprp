...
from undp_nuprp.approvals.models import TotalSaving
from undp_nuprp.reports.models import TotalSavingMapReport
from undp_nuprp.reports.views.interactive_maps.interactive_map_report import GenericInteractiveMapReportView

__author__ = 'Kaikobud'


@decorate(override_view(model=TotalSavingMapReport, view=ViewActionEnum.Manage))
class TotalSavingMapReportView(GenericInteractiveMapReportView):
    def get_template_names(self):
        return ['reports/interactive-map-city.html']

    def get_report_parameters(self, **kwargs):
        parameters = super(TotalSavingMapReportView, self).get_report_parameters(**kwargs)
    ...
    def get_context_data(self, **kwargs):
        context = super(TotalSavingMapReportView, self).get_context_data(**kwargs)
        context['title'] = "Value of Total Savings"
        ...
        context['legend_label'] = "Total Savings (BDT)"
        return context

    @classmethod
    def prepare_color_range(cls, city_ids):
        queryset = TotalSaving.objects.values('city_id').annotate(Sum(F('value_of_total_savings')))
        ...
        if queryset.exists():
            _key = 'value_of_total_savings__sum'
    ...
    def prepare_city_wise_total_data(cls, city_ids):
        _queryset = TotalSaving.objects.values('city_id').annotate(Sum(F('value_of_total_savings')))
        ...
        for _item in _queryset:
            result[_item['city_id']][total_key] = float(_item['value_of_total_savings__sum']) or 0

        return result

    @classmethod
    def prepare_value_of_total_savings_data(cls, city_ids=None):

        queryset = TotalSaving.objects.filter(city__isnull=False)
        if city_ids:
        ...
        result = dict()
        _key = 'Value of Total Savings'
        for geography_id in geography_ids:
            result[geography_id] = {
                _key: 0,
            }

        _queryset = queryset.values('city_id').annotate(Sum(F('value_of_total_savings')))

        for _item in _queryset:
            value_of_total_savings = _item['value_of_total_savings__sum'] or 0
            result[_item['city_id']][_key] = value_of_total_savings
    ...
    def get_json_response(self, content, **kwargs):
        ...
        data_dict = dict()
        data_dict['title'] = "Value of Total Savings"
        data_dict['data'] = self.prepare_value_of_total_savings_data(city_ids)

        return super(TotalSavingMapReportView, self).get_json_response(
