...
from undp_nuprp.approvals.models import MobilizedPrimaryGroupMember
from undp_nuprp.reports.models import MobilizedPrimaryGroupMemberMapReport
from undp_nuprp.reports.views.interactive_maps.interactive_map_report import GenericInteractiveMapReportView

__author__ = 'Kaikobud'


@decorate(override_view(model=MobilizedPrimaryGroupMemberMapReport, view=ViewActionEnum.Manage))
class MobilizedPrimaryGroupMemberMapReportView(GenericInteractiveMapReportView):
    def get_template_names(self):
        return ['reports/interactive-map-city-ward.html']

    def get_report_parameters(self, **kwargs):
        request = CrequestMiddleware.get_request()
        parameters = super(MobilizedPrimaryGroupMemberMapReportView, self).get_report_parameters(**kwargs)
        user = request.c_user
    ...
    def get_context_data(self, **kwargs):
        context = super(MobilizedPrimaryGroupMemberMapReportView, self).get_context_data(**kwargs)
        context['title'] = "Number of Registered Primary Group Households"
        ...
        context['legend_label'] = "Number of PG Households"
        return context

    @classmethod
    def prepare_color_range(cls, city_ids):
        queryset = MobilizedPrimaryGroupMember.objects.values(
            'city_id'
        ).annotate(Sum(F('number_of_mobilized_primary_group_members')))
        ...
        if queryset.exists():
            _key = 'number_of_mobilized_primary_group_members__sum'
        ...
        queryset_for_ward = MobilizedPrimaryGroupMember.objects.exclude(
            ward__isnull=True
        ).values('city_id', 'ward_id', 'ward__name').annotate(Sum('number_of_mobilized_primary_group_members'))
        ...
        if queryset_for_ward.exists():
            ...
            if range_diff <= 3:
                ...
                for q in queryset_for_ward:
                    ward_id = ward_id_dict.get(str(q['ward__name']) + '_' + str(q['city_id']))
                    if ward_id:
                        count = q[_key] or 0
    ...
    def prepare_city_wise_total_data(cls, city_ids):
        _queryset = MobilizedPrimaryGroupMember.objects.values(
            'city_id'
        ).annotate(Sum(F('number_of_mobilized_primary_group_members')))
        ...
        for _item in _queryset:
            result[_item['city_id']][total_key] = _item['number_of_mobilized_primary_group_members__sum'] or 0

        return result

    @classmethod
    def prepare_mobilized_primary_group_members_data(cls, ward_ids=None, city_ids=None):

        queryset = MobilizedPrimaryGroupMember.objects.filter(city__isnull=False)
        if ward_ids:
        ...
        result = dict()
        _key = 'Number of Registered Primary Group Households'
        ...
        _queryset = queryset.values('city_id', 'ward__name').annotate(Sum(F('number_of_mobilized_primary_group_members')))

        for _item in _queryset:
            number_of_mobilized_primary_group_members = _item['number_of_mobilized_primary_group_members__sum'] or 0
            ward_no = geography_id_dict.get(str(_item['ward__name']) + '_' + str(_item['city_id'])) \
                      or geography_id_dict.get((str(_item['ward__name']).zfill(2)) + '_' + str(_item['city_id']))
            if ward_no:
                result[ward_no][_key] = number_of_mobilized_primary_group_members
                result[_item['city_id']][_key] += number_of_mobilized_primary_group_members
    ...
    def get_json_response(self, content, **kwargs):
        ...
        data_dict = dict()
        data_dict['title'] = "Number of Registered Primary Group Households"
        data_dict['data'] = self.prepare_mobilized_primary_group_members_data(ward_ids, city_ids)

        return super(MobilizedPrimaryGroupMemberMapReportView, self).get_json_response(
