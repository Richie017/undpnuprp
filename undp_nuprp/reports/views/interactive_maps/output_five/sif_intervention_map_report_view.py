...
from undp_nuprp.reports.models import SIFInterventionMapReport
...
@decorate(override_view(model=SIFInterventionMapReport, view=ViewActionEnum.Manage))
class SIFInterventionMapReportView(InterventionMapReportView):

    def get_report_parameters(self, **kwargs):
        request = CrequestMiddleware.get_request()
        parameters = super(SIFInterventionMapReportView, self).get_report_parameters(**kwargs)
        user = request.c_user
    ...
    def get_context_data(self, **kwargs):
        context = super(SIFInterventionMapReportView, self).get_context_data(**kwargs)
        context['title'] = "SIF Interventions"
    ...
    def prepare_color_range(cls, city_ids):
        ...
        queryset = Intervention.objects.filter(
            ...
            'sif__assigned_cdc__address__geography__parent_id'
        ).annotate(Sum('number_of_facilities'))

        city_dict = dict()
        ward_dict = dict()
        for q in queryset:
            ward_dict[q['sif__assigned_cdc__address__geography_id']] = q['number_of_facilities__sum'] or 0
            if q['sif__assigned_cdc__address__geography__parent_id'] not in city_dict.keys():
                city_dict[q['sif__assigned_cdc__address__geography__parent_id']] = 0
            city_dict[q['sif__assigned_cdc__address__geography__parent_id']] += q['number_of_facilities__sum'] or 0

        # deal with cdc cluster data
        queryset = Intervention.objects.filter(
            type_of_intervention__in=all_intervention_type,
            sif__assigned_cdc_cluster__address__geography__isnull=False,
            sif__assigned_cdc__address__geography__isnull=True
        ).distinct().values(
            'sif__assigned_cdc_cluster__address__geography_id'
        ).annotate(Sum('number_of_facilities'))

        for q in queryset:
            if q['sif__assigned_cdc_cluster__address__geography_id'] not in city_dict.keys():
                city_dict[q['sif__assigned_cdc_cluster__address__geography_id']] = 0
            city_dict[q['sif__assigned_cdc_cluster__address__geography_id']] += q['number_of_facilities__sum'] or 0
    ...
    def prepare_city_wise_total_data(cls, city_ids):
        ...
        for _item in queryset:
            result[
                _item['sif__assigned_cdc__address__geography__parent_id']
        ...
        for _item in queryset:
            result[
                _item['sif__assigned_cdc_cluster__address__geography_id']
            ][total_key] += _item['number_of_facilities__sum'] or 0

        return result

    @classmethod
    def prepare_sif_intervention_data(cls, ward_ids=None, city_ids=None):
        latrine = [
            'Single pit latrine',
            'Twin pit latrine',
            'Community Latrine'
        ...
        intervention_type_dict = {
            ...
            'Length of Footpath (in meters)': footpath
        }

        annotation_dict = {
            'Number of Latrine': 'number_of_facilities',
            'Number of Tubewell': 'number_of_facilities',
            'Length of Drain (in meters)': 'length',
            'Length of Footpath (in meters)': 'length',
        }
        ...
        if ward_ids:
            queryset = queryset.filter(sif__assigned_cdc__address__geography_id__in=ward_ids)
            geography_ids = ward_ids + city_ids
        elif city_ids:
            queryset = queryset.filter(sif__assigned_cdc__address__geography__parent_id__in=city_ids)
        ...
        for geography_id in geography_ids:
            result[geography_id] = {
                ...
                'Length of Footpath (in meters)': 0
            }
        ...
        for _key, _types in intervention_type_dict.items():
            ...
            for _item in _queryset:
                ...
                result[_item['sif__assigned_cdc__address__geography__parent_id']][_key] += total_facilities
        ...
        if city_ids:
            queryset = queryset.filter(sif__assigned_cdc_cluster__address__geography_id__in=city_ids)
        ...
        for _key, _types in intervention_type_dict.items():
            ...
            for _item in _queryset:
                total_facilities = _item['{}__sum'.format(_annotator)] or 0
                result[_item['sif__assigned_cdc_cluster__address__geography_id']][_key] += total_facilities
    ...
    def get_json_response(self, content, **kwargs):
        ...
        data_dict = dict()
        data_dict['title'] = "SIF Interventions Location Map"
        data_dict['data'] = self.prepare_sif_intervention_data(ward_ids, city_ids)
        data_dict['location_data'] = self.prepare_intervention_location_data(report_type="SIF", city_ids=city_ids)

        return super(SIFInterventionMapReportView, self).get_json_response(
