...
from undp_nuprp.reports.config.infrastructure_sif_and_crmif_indicators import infrastructure_sif_and_crmif_indicator
...
@decorate(override_view(model=InfrastructureSIFAndCRMIFReport, view=ViewActionEnum.Manage))
class InfrastructureSIFAndCRMIFView(GenericReportView):
    def get(self, request, *args, **kwargs):
        return super(InfrastructureSIFAndCRMIFView, self).get(request, *args, **kwargs)

    def get_indicators(self, **kwargs):
        return infrastructure_sif_and_crmif_indicator

    def get_context_data(self, **kwargs):
        context = super(InfrastructureSIFAndCRMIFView, self).get_context_data(**kwargs)
        context['title'] = "Infrastructure (SIF & CRMIF)"
        context['indicators'] = self.get_indicators(**kwargs)
        return context

    def get_report_parameters(self, **kwargs):
        parameters = super(InfrastructureSIFAndCRMIFView, self).get_report_parameters(**kwargs)
    ...
    def get_json_response(self, content, **kwargs):
        ...
        report_data = InfrastructureSIFAndCRMIFReport().build_report(from_time=f_date, to_time=t_date, wards=ward_ids,
        ...
        return super(InfrastructureSIFAndCRMIFView, self).get_json_response(self.convert_context_to_json(response_dict),
                                                                            **kwargs)
