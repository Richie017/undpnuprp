...
from undp_nuprp.reports.config.o_and_m_funds_indicators import o_and_m_funds_indicator
...
@decorate(override_view(model=OAndMFundsReport, view=ViewActionEnum.Manage))
class OAndMFundsView(GenericReportView):
    def get(self, request, *args, **kwargs):
        return super(OAndMFundsView, self).get(request, *args, **kwargs)

    def get_indicators(self, **kwargs):
        return o_and_m_funds_indicator

    def get_context_data(self, **kwargs):
        context = super(OAndMFundsView, self).get_context_data(**kwargs)
        context['title'] = "O&M Funds"
        context['indicators'] = self.get_indicators(**kwargs)
        return context

    def get_report_parameters(self, **kwargs):
        parameters = super(OAndMFundsView, self).get_report_parameters(**kwargs)
    ...
    def get_json_response(self, content, **kwargs):
        ...
        report_data = OAndMFundsReport().build_report(from_time=f_date, to_time=t_date, wards=ward_ids,
        ...
        return super(OAndMFundsView, self).get_json_response(self.convert_context_to_json(response_dict), **kwargs)
