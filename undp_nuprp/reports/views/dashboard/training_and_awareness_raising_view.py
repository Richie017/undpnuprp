...
from undp_nuprp.reports.config.training_and_awareness_raising_indicators import training_and_awareness_raising_indicator
...
@decorate(override_view(model=TrainingAndAwarenessRaisingReport, view=ViewActionEnum.Manage))
class TrainingAndAwarenessRaisingView(GenericReportView):
    def get(self, request, *args, **kwargs):
        return super(TrainingAndAwarenessRaisingView, self).get(request, *args, **kwargs)

    def get_indicators(self, **kwargs):
        return training_and_awareness_raising_indicator

    def get_context_data(self, **kwargs):
        context = super(TrainingAndAwarenessRaisingView, self).get_context_data(**kwargs)
        context['title'] = "Training and Awareness Raising"
        context['indicators'] = self.get_indicators(**kwargs)
        return context

    def get_report_parameters(self, **kwargs):
        parameters = super(TrainingAndAwarenessRaisingView, self).get_report_parameters(**kwargs)
    ...
    def get_json_response(self, content, **kwargs):
        ...
        #     f_date = datetime.strptime(from_date, "%d/%m/%Y").timestamp() * 1000
        # else:
        #     f_date = 0
        #
        # if to_date is not None:
        #     t_date = (datetime.strptime(to_date, "%d/%m/%Y") + timedelta(1)).timestamp() * 1000
        # else:
        #     t_date = SurveyResponse.objects.aggregate(max_time=Max('survey_time'))['max_time']
        ...
        report_data = TrainingAndAwarenessRaisingReport().build_report(from_time=f_date, to_time=t_date, wards=ward_ids,
        ...
        return super(TrainingAndAwarenessRaisingView, self).get_json_response(self.convert_context_to_json(response_dict), **kwargs)
