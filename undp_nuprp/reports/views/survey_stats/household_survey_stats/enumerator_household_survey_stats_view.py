...
from undp_nuprp.reports.models.survey_stats.household_survey_stats.enumerator_household_survey_stats import \
    EnumeratorHouseholdSurveyStats
...
@decorate(override_view(model=EnumeratorHouseholdSurveyStats, view=ViewActionEnum.Manage))
class EnumeratorHouseholdSurveyStatsView(GenericReportView):
    def get(self, request, *args, **kwargs):
        if request.GET.get('export', 'False') == 'True':
            ...
            dest_filename = self.__class__.generate_excel(
                request, surveys=survey_ids, cities=city_ids,
                wards=ward_ids, domain=selected_wards,
                time_from=f_date, time_to=t_date
            )
            if dest_filename is not None:
                if self.is_json_request(request):
                    return super(EnumeratorHouseholdSurveyStatsView, self).get_json_response(
        ...
        return super(EnumeratorHouseholdSurveyStatsView, self).get(request, *args, **kwargs)

    def get_context_data(self, **kwargs):
        context = super(EnumeratorHouseholdSurveyStatsView, self).get_context_data(**kwargs)
        context['title'] = "Enumerator Survey Stats"
        return context

    def get_report_parameters(self, **kwargs):
        parameters = super(EnumeratorHouseholdSurveyStatsView, self).get_report_parameters(**kwargs)
    ...
    def get_json_response(self, content, **kwargs):
        ...
        report, fixed_columns = EnumeratorHouseholdSurveyStats.build_report(
    ...
    def generate_excel(cls, request, surveys=None, cities=None, wards=None, domain=None, time_from=None,
        ...
        try:
            ...
            if not os.path.exists(path):
                os.makedirs(path)
            wb = Workbook()
            dest_filename = 'Enumerator_Survey_Stats_' + str(Clock.utcnow().strftime('%d%m%Y%H%M'))
            file_path = path + os.sep + dest_filename + '.xlsx'
            ws = wb.create_sheet(index=0)
            row_number = 1
            column_number = 1
            ws.title = 'Enumerator Survey Stats'
            title_cell_style = ws.cell(row=row_number, column=column_number)
            title_cell_style.value = "Enumerator Survey Stats"
            ...
            report, fixed_columns = EnumeratorHouseholdSurveyStats.build_report(
                ...
                wards=wards, domain=domain, time_from=time_from,
                time_to=time_to, styled=True
            )
