...
def get_on_budget_completed_intervention_column_chart(wards=list(), from_time=None, to_time=None):
    ...
    for key in intervention_types:
        ...
        percentage = total_within_budget / total_crmif * 100 if total_crmif else 0
...
def get_on_budget_completed_intervention_table_data(wards=list(), from_time=None, to_time=None):
    ...
    for key in intervention_types:
        ...
        for completed_intervention_info in completed_intervention_queryset:
            ...
            _int_within_bud, _int_tot_crmif = intervention_wise_data[key]
            _int_within_bud += _total_within_budget
            _int_tot_crmif += _total_crmif
            intervention_wise_data[key] = (_int_within_bud, _int_tot_crmif)

            _tot_within_bud, _tot_crmif = city_data[_city][key]
            _tot_within_bud += _total_within_budget
            _tot_crmif += _total_crmif
            city_data[_city][key] = (_tot_within_bud, _tot_crmif)
    ...
    for city in cities:
        ...
        for intervention_type in intervention_types:
            _total_within_budget, _total_crmif = city_data[city][
                intervention_type] if city in city_data and intervention_type in city_data[city] else (0, 0)
            percent = _total_within_budget / _total_crmif * 100 if _total_crmif else 0
    ...
    row_data = ['Total']
    for intervention_type in intervention_types:
        _total_within_budget, _total_crmif = intervention_wise_data[
