...
def get_beneficiary_by_intervention_stacked_column_chart(wards=list(), from_time=None, to_time=None):
    ...
    for key in intervention_types:
        ...
        beneficiary_dict = crmif_queryset.filter(interventions__type_of_intervention__in=intervention_type).aggregate(
...
def get_beneficiary_by_intervention_table_data(wards=list(), from_time=None, to_time=None):
    ...
    table_data.append(['Total', '{}% ({})'.format(round(grand_total_pg_beneficiary_percent), grand_total_pg_beneficiary),
