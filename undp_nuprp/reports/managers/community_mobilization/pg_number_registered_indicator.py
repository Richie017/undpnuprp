...
def get_pg_number_registered_indicator_table_data(wards=list(), from_time=None, to_time=None):
    ...
    queryset = pg_member_queryset.values('assigned_to__parent__address__geography__parent__name'). \
        ...
        annotate(count=Count('assigned_to_id', distinct=True))
...
def get_pg_number_registered_indicator_flat_data(wards=list(), from_time=None, to_time=None):
    ...
    queryset = pg_members.filter(assigned_to__parent__address__geography__parent__isnull=False). \
        aggregate(pg_count=Count('assigned_to_id', distinct=True))
