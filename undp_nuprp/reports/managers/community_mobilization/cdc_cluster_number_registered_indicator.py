...
def get_cdc_cluster_registered_indicator_table_data(wards=list(), from_time=None, to_time=None):
    ...
    queryset = pg_member_queryset.values('assigned_to__parent__address__geography__parent__name'). \
        ...
        annotate(count=Count('assigned_to__parent__parent_id', distinct=True))
    ...
    for data in queryset:
        ...
        if city is not None:
            ...
            total_cdc_cluster += city_dict[city]

    response_data = list()
    total_row = ['Total (all cities)', str(total_cdc_cluster)]
...
def get_cdc_cluster_registered_indicator_flat_data(wards=list(), from_time=None, to_time=None):
    ...
    queryset = pg_members.filter(assigned_to__parent__address__geography__parent__isnull=False). \
        aggregate(cluster_count=Count('assigned_to__parent__parent_id', distinct=True))

    total_cdc_cluster = queryset['cluster_count']
    if total_cdc_cluster is None:
        total_cdc_cluster = 0
    return '<h1 style="font-weight:bold">Number of CDC Clusters in which members registered (all cities)</h1><div><span style="font-size: 36px;">' \
           + str(total_cdc_cluster) + '</span></div>'
