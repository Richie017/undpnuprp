...
def get_employment_indicator_pie_chart_data(wards=list(), from_time=None, to_time=None):
    ...
    accepted_answers = list(Answer.objects.using(BWDatabaseRouter.get_read_database_name()).filter(
        question__question_code='1.9.').values_list('text', flat=True))
...
def get_employment_indicator_column_chart_data(wards=list(), from_time=None, to_time=None):
    role_wise_queryset = QuestionResponseCache.get_role_based_queryset(
        queryset=QuestionResponseCache.objects.filter()
    ).using(BWDatabaseRouter.get_read_database_name())
    question_responses = role_wise_queryset.filter(
    ...
    top_six_answers = list(question_responses.filter(question_code='1.9.').values('answer_text'). \
    ...
    queryset = question_responses.filter(answer_text__in=accepted_answers).values(
        ...
        count=Sum('survey_count')).order_by('-count', 'city_name', 'answer_text')

    other_queryset = question_responses.order_by('answer__order').exclude(answer_text__in=accepted_answers).values(
        'city_name').annotate(count=Sum('survey_count')).order_by('city_name')
...
def get_employment_indicator_table_data(wards=list(), from_time=None, to_time=None):
    role_wise_queryset = QuestionResponseCache.get_role_based_queryset(
        queryset=QuestionResponseCache.objects.filter()
    ).using(BWDatabaseRouter.get_read_database_name())
    question_responses = role_wise_queryset.filter(
    ...
    response_data = list()
    response_data.append((['Employment of HH Head', ] + [emp for emp in city_dict.keys()]))
