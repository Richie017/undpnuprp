...
def get_pg_employment_status_indicator_pie_chart_data(wards=list(), from_time=None, to_time=None):
    ...
    all_answers = list(question_responses.values('employment_counts__label')
                       .annotate(count=Sum('employment_counts__count'))
                       .order_by('-count').values_list('employment_counts__label', flat=True))
    ...
    queryset = question_responses.filter(employment_counts__label__in=accepted_answers).values(
        'employment_counts__label').annotate(
        count=Sum('employment_counts__count')).order_by('-count', 'employment_counts__label')

    other_queryset = question_responses.filter(employment_counts__label__in=other_answers). \
        aggregate(count=Sum('employment_counts__count'))

    report_data = list()
    for answer in queryset:
        employment = answer['employment_counts__label']
        if not employment:
            continue
...
def get_pg_employment_status_indicator_table_data(wards=list(), from_time=None, to_time=None):
    question_responses = PGMemberInfoCache.objects.all().using(BWDatabaseRouter.get_read_database_name())
    if from_time and to_time:
        question_responses = question_responses.filter(
            from_time__gte=from_time - 1000, to_time__lte=to_time + 1000)
    ...
    queryset = question_responses.values('employment_counts__label', 'city__name').annotate(
        count=Sum('employment_counts__count')).order_by('-count', 'city__name', 'employment_counts__label')

    all_queryset = question_responses.values('city__name', 'employment_counts__label').order_by(
        'city__name', 'employment_counts__label').annotate(count=Sum('employment_counts__count'))
    ...
    for data in queryset:
        answer = data.get('employment_counts__label')
