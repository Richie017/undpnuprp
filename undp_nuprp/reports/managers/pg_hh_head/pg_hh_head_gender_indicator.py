...
def get_hhgender_indicator_pie_chart_data(wards=list(), from_time=None, to_time=None):
    ...
    queryset = question_responses.values('hh_gender_counts__label').order_by(
        'hh_gender_counts__label').annotate(count=Sum('hh_gender_counts__count'))
    ...
    for answer in queryset:
        gender = answer['hh_gender_counts__label']
        if not gender:
            continue
...
def get_hhgender_indicator_table_data(wards=list(), from_time=None, to_time=None):
    question_responses = PGMemberInfoCache.objects.all().using(BWDatabaseRouter.get_read_database_name())
    if from_time and to_time:
        question_responses = question_responses.filter(
            from_time__gte=from_time - 1000, to_time__lte=to_time + 1000)
    if wards:
        question_responses = question_responses.filter(ward_id__in=wards)
    queryset = question_responses.values('city__name', 'hh_gender_counts__label').order_by(
        'city__name', 'hh_gender_counts__label').annotate(count=Sum('hh_gender_counts__count'))
    ...
    for data in queryset:
        answer = data.get('hh_gender_counts__label')
