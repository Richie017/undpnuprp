...
from undp_nuprp.reports.config.constants.pg_survey_constants import HH_EDUCATION_ATTAINMENT_INDICATORS
...
def get_hh_head_education_attainment_indicator_pie_chart_data(wards=list(), from_time=None, to_time=None):
    ...
    all_answers = list(question_responses.values('hh_education_counts__label'). \
                       annotate(count=Sum('hh_education_counts__count')).order_by('-count').
                       values_list('hh_education_counts__label', flat=True))
    ...
    queryset = question_responses.filter(hh_education_counts__label__in=accepted_answers).values(
        'hh_education_counts__label').annotate(
        count=Sum('hh_education_counts__count')).order_by('-count', 'hh_education_counts__label')

    other_queryset = question_responses.filter(hh_education_counts__label__in=other_answers). \
        aggregate(count=Sum('hh_education_counts__count'))

    report_data = list()
    for answer in queryset:
        education = answer['hh_education_counts__label']
        if not education:
            continue
        report_data.append({
            'name': education,
...
def get_hh_head_education_attainment_indicator_table_data(wards=list(), from_time=None, to_time=None):
    question_responses = PGMemberInfoCache.objects.all().using(BWDatabaseRouter.get_read_database_name())
    if from_time and to_time:
        question_responses = question_responses.filter(from_time__gte=from_time - 1000, to_time__lte=to_time + 1000)
    if wards:
    ...
    queryset = question_responses.values('hh_education_counts__label', 'city__name').annotate(
        count=Sum('hh_education_counts__count')).order_by('-count', 'city__name', 'hh_education_counts__label')

    all_queryset = question_responses.values('city__name', 'hh_education_counts__label').order_by(
        'city__name', 'hh_education_counts__label').annotate(count=Sum('hh_education_counts__count'))

    education_attainment_dict = OrderedDict()
    education_attainment_indicators = HH_EDUCATION_ATTAINMENT_INDICATORS()
    for education_attainment in education_attainment_indicators:
        education_attainment_dict[education_attainment] = 0

    for data in queryset:
        answer = data.get('hh_education_counts__label')
        ...
        education_attainment_dict[answer] += data.get('count')
