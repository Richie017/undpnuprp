...
from undp_nuprp.approvals.models import SIF


def get_interventions_completed_less_than_half(wards=list, from_time=None, to_time=None):
    ...
    for key in intervention_types:
        ...
        head.append(key)
        head.append(key + ' (Avg %)')
        sifs = sif_queryset.filter(interventions__type_of_intervention__in=type_,
        ...
        for sif in sifs:
            total_percentage = sif['total_percentage']
            total_interventions = sif['total_interventions']
            city = sif['assigned_city'] or 'Unassigned'
