...
def get_on_time_completed_intervention_column_chart(wards=list(), from_time=None, to_time=None):
    ...
    for key in intervention_types:
        ...
        percentage = total_on_time / total_sif * 100 if total_sif else 0
...
def get_on_time_completed_intervention_table_data(wards=list(), from_time=None, to_time=None):
    ...
    for key in intervention_types:
        ...
        for completed_intervention_info in completed_intervention_queryset:
            ...
            _total_sif = completed_intervention_info['total_sif'] if completed_intervention_info[
                'total_sif'] else 0
            ...
            _int_tot_time, _int_tot_sif = intervention_wise_data[key]
            _int_tot_time += _total_on_time
            _int_tot_sif += _total_sif
            intervention_wise_data[key] = (_int_tot_time, _int_tot_sif)

            _tot_time, _tot_sif = city_data[_city][key]
            _tot_time += _total_on_time
            _tot_sif += _total_sif
            city_data[_city][key] = (_tot_time, _tot_sif)
    ...
    for city in cities:
        ...
        for intervention_type in intervention_types:
            _total_on_time, _total_sif = city_data[city][
                intervention_type] if city in city_data and intervention_type in city_data[city] else (0, 0)
            percent = _total_on_time / _total_sif * 100 if _total_sif else 0
    ...
    row_data = ['Total']
    for intervention_type in intervention_types:
        _total_on_time, _total_sif = intervention_wise_data[
