...
def get_community_contract_flat_data(wards=list(), from_time=None, to_time=None):
    total_number_of_contract = SIF.objects.count()

    return '<h1 style="font-weight:bold">Number of community contracts (all cities)</h1>' \
...
def get_community_contract_table_data(wards=list(), from_time=None, to_time=None):
    ...
    city_wise_community_numbers = sif_queryset.values('assigned_city').annotate(total=Count('id')).order_by(
        'assigned_city')

    total_community_number = sif_queryset.count()

    table_data = list()
    table_data.append(['City/Town', ] + [{
        'column_name': 'Number of Community Contracts',
        'extra_column_name': 'Number of Community Contracts' + "(%)",
        'split': 'true'
    }])
